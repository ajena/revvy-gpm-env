...
BEGIN
    ...
    IF LENGTH(sf_namespace) = 0 THEN
        RAISE NOTICE '%: namespace not specified', now();
    ELSE
        RAISE NOTICE '%: namespace: %', now(), sf_namespace;
    END IF;


    --  1. Pause INSERT/DELETE on mnadvsimulation__c table.
    RAISE NOTICE '%: Pausing INSERT/DELETE on %', now(), master_table_name;
    script_text := 'CREATE RULE ' || master_table_name || '_insert_disable AS ON INSERT TO gpm.' || master_table_name || ' DO INSTEAD NOTHING';
    RAISE NOTICE '%: %', now(), script_text;
    EXECUTE script_text;
    ...
    --  3. Create a backup of mnadvsimulationoutput__c table.
    --     Name the new table like this - mnadvsimulationoutput__c_bak_<current_datetime>.
    RAISE NOTICE '%: Creating backup of %', now(), master_table_name;
    ...
    --  4. Create a new table (without data) from mnadvsimulationoutput__c table.
    RAISE NOTICE '%: Creating a structural copy of %', now(), master_table_name;
    ...
    --  5. Add column mnadvsimcreateddate__c to mnadvsimulationoutput__c_new to capture the simulation scenario creation date.
    --     This new column will act as the partition key for the quarterly range partition.
    RAISE NOTICE '%: Adding column % to %', now(), date_range_part_col_name, new_master_table_name;
    ...
    --     - create child tables(with PK constraint) of mnadvsimulationoutput__c_new master table.
    --     - insert appropriate rows into the child table
    RAISE NOTICE '%: Start creation of child tables.', now();
    -- Determine start year/quarter
    RAISE NOTICE '%: Determine start year/quarter.', now();
    ...
    RAISE NOTICE '%: %', now(), start_year || ' Q' || start_quarter;

    -- Determine end year/quarter
    RAISE NOTICE '%: Determine end year/quarter.', now();
    curr_year := extract(year FROM now());
    curr_quarter := extract(quarter FROM now());
    RAISE NOTICE '%: %', now(), curr_year || ' Q' || curr_quarter;

    RAISE NOTICE '%: Creating child tables starting from year & quarter : %', now(), start_year || ' Q' || start_quarter;
    WHILE (start_year * 10 + start_quarter) <= (curr_year * 10 + curr_quarter) LOOP
        ...
        RAISE NOTICE '%: Creating child table', now();
        ...
        -- Insert rows
        RAISE NOTICE '%: Inserting rows into child table', now();
    ...
    RAISE NOTICE '%: Child tables created till year & quarter : %', now(), curr_year || ' Q' || curr_quarter;


    --  7. Verify partitioned data integrity
    RAISE NOTICE '%: Running Verification...', now();
    script_text := 'SELECT COUNT(1) '
                || 'FROM gpm.' || master_table_name || ' aso '
                || 'INNER JOIN gpm.' || sim_table_name || ' asim ON aso.' || sim_table_fk_name || ' = asim.sfid';
    RAISE NOTICE '%: %', now(), script_text;
    EXECUTE script_text INTO source_row_count;
    RAISE NOTICE '%: %', now(), source_row_count;
    script_text := 'SELECT COUNT(1) '
    ...
    RAISE NOTICE '%: %', now(), row_count;
    IF row_count > 0 THEN
        RAISE EXCEPTION '%: After paritioning, table % should not contain any records.', now(), new_master_table_name;
    END IF;


    --  8. Create Indexes, INSERT rules and CHECK constraints.
    RAISE NOTICE '%: Creating INSERT Rules and Check constraints.', now();
    ...
    WHILE (start_year * 10 + start_quarter) <= (curr_year * 10 + curr_quarter) LOOP
        ...
        -- Create indexes
        RAISE NOTICE '%: Creating indexes', now();
        child_table_name_for_index := sf_namespace || master_table_short_name || '_' || start_year || 'q' || start_quarter;
        RAISE NOTICE '%: Creating Index - %', now(), 'idx_' || child_table_name_for_index || '_sim_countryorder_cpsku_cpt ON ' || child_table_name
        ...
        RAISE NOTICE '%: Creating Index - %', now(), 'idx_' || child_table_name_for_index || '_forecastdate_sim ON ' || child_table_name
        ...
        RAISE NOTICE '%: Creating Index - %', now(), 'idx_' || child_table_name_for_index || '_partkey ON ' || child_table_name
    ...
    --  9. Rename tables so that mnadvsimulation__c_new becomes mnadvsimulation__c
    RAISE NOTICE '%: Renaming tables.', now();
    ...
    -- 10. Turn on constraint_exclusion.
    RAISE NOTICE '%: Turning on constraint exclusion', now();
    SET constraint_exclusion = on;


    -- 11. Resume INSERT/DELETE on mnadvsimulation__c table.
    RAISE NOTICE '%: Resuming INSERT/DELETE on %', now(), master_table_name;
    ...
    RAISE NOTICE '%: END OF PARTITION_MNADVSIMULATIONOUTPUT', now();
